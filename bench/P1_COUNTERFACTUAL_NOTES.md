...
### 5.4 判分有没有放宽过头（架构台四头验）

改同义词之后必须确认尺子还分得出对错，否则 off 的 3/3 不可信。四头：

| 头 | 输入 | 结果 |
|---|---|---|
| 废话 | 六行与违规无关的话 | 六条全挂（`test_fourway_nonsense_all_six_fail`） |
| 中文 | 实跑 off t2 / on t1 原文 | 过 |
| 英文 | `solution/AUDIT.md` | 过 |
| 互换 | 把 alpha/beta 两行对调 | 只有这两条挂，其余仍过（`test_fourway_swap_alpha_beta_only_those_fail`） |

判别力还在。off 3/3 建立在这把尺子上，不是「放宽到怎么写都过」。
