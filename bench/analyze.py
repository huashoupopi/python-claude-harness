...
def block_anomalies(rows):
    ...
    buckets = {
        ...
        # 🔴 2026-08-16 加这一桶:凭空造文件是【幻觉螺旋】的指纹,比行数更锐利。
        # 实测 08-15 那批:files_created 中位 0 / 90 分位 1 / 最大 40 ——
        # 那一条(t08 幻觉出 40 个不存在的校验器)是【40 倍】离群点,
        # 而按行数看是 134 vs 阈值 60,只有 2 倍多,差点淹在正常改动里。
        # 🪝 同一个异常,换个指标就从「勉强越线」变成「一眼可见」——
        #    指标选得准，阈值就不用调得巧。
        "凭空造文件(>3 个)": [r for r in rows if r.get("files_created", 0) > 3],
    }
    clean = True
...
def main():
    ...
    print(
        ...
        "\n⚠️ 记忆轴怎么读 —— 分两种题,别混着讲(2026-08-16 更正,旧版这段话现在是错的):"
        "\n   · 单轮题:考场每次全新、.memory/ 是空的 → 收益【结构性地为零】,"
        "\n     这些题上的记忆轴数据不携带信息,不能拿来说「memory 没用」。"
        "\n   · 多轮题(有 round2.md 的):第一轮的记忆留在考场里,实测 memory_injected 100%,"
        "\n     记忆层【确实在工作】。这些题上测出来的零,是【真的零结果】。"
        "\n   📌 但真零结果也有作用域:多轮题里第二轮还能看见【上一轮改过的代码】,"
        "\n     所以测的是「要不要重新推一遍」,不是「记不记得住不可能知道的事」。"
        "\n     见 STAGE3_NOTES.md §2.3。"
    )
