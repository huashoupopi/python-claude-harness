...
def complete_task(task_id: str) -> str:
    ...
    if task.status != TaskStatus.IN_PROGRESS:
        return f"Task {task_id} is {task.status}, cannot be completed."
...
def is_slow_operation(tc_name: str, tc_args: dict) -> bool:
    ...
    return any(kws in cmd for kws in slow_keywords)


def should_run_in_background(tc_name: str, tc_args: dict) -> bool:
...
def execute_tool(tc) -> str:
    ...
    if not entry:
        ...
        try:
            ...
            print(
                f"  \033[35m[tool call] {tc.function.name}({args.model_dump()})\033[0m"
            )
            result = handler(**args.model_dump())
    return result


def start_background_task(tc, args: dict) -> str:
    global _bg_counter
    _bg_counter += 1
    ...
    def worker():
        print(f"  \033[33m[background start] {bg_id}: {cmd}\033[0m")
...
def agent_loop(messages: list, context: dict):
    ...
    for turn in range(max_turns):
        ...
        for tc in msg.tool_calls:
            ...
            if should_run_in_background(tc.function.name, args):
                bg_id = start_background_task(tc, args)
                messages.append(
                    {
                        "role": "tool",
                        "tool_call_id": tc.id,
                        "content": f"[Background task {bg_id} started] Command: {args.get('command', tc.function.name)}. Result will be available when complete.",
                    }
                )
                continue
            result = execute_tool(tc)
