...
> ⚖️ **2026-08-20 架构台裁定：暂缓，不在 9 月投递前做。08-21 落盘。**
>
> **不推翻 08-16 那句「要做」。** 当事人的判断（3900 行单文件的结构不好）依然成立，
> 改的只是**时机**——原本排在封版前，现在推到投递之后。
>
> **理由是探针实测查出的一条原 backlog 未记的风险**（详见下方 🔴 条）：
> `conftest.py:26-42` 的 sandbox fixture 靠 `monkeypatch.setattr(trunk, "TASKS_DIR", ...)`
> 换模块全局来做隔离。拆包后若用 `from pkg import *` 做 facade，patch 打在影子绑定上，
> **45 个测试的隔离会静默失效**——不是测试变红，是照常绿灯然后写进真实目录。已构造最小用例实证。
>
> **代价核算**：拆包的收益是可读性；但 README 已经给了 13 层代码地图 + `【第 N 层】` 搜索锚点，
> 可读性**已被文档兜住大半**，边际收益因此变小。而风险是在距投递 3 周时，
> 把唯一一套能证明 agent 行为的测试悄悄弄坏。**收益变小、风险不变 → 推后。**
>
> **解除条件**：先把 sandbox fixture 从「patch 模块全局」改成「显式传参 / fixture 注入」，
> 让隔离不再依赖「函数与被 patch 的名字在同一个模块对象上」这个前提，**再**动文件结构。
> 顺序反了就是拿 45 个测试赌运气。
